const state = {
  ...
  flash: { view: 'grid', pool: [], singleQueue: [], singleIndex: 0, singleFlipped: false, singleDone: false },
...
function dirLabelZh(){ return state.settings.reverseDirection ? '越文 → 中文' : '中文 → 越文'; }
...
function speak(text){ if(!('speechSynthesis' in window) || !text) return; const u = new SpeechSynthesisUtterance(text); u.lang='zh-TW'; u.rate=0.9; window.speechSynthesis.cancel(); window.speechSynthesis.speak(u); }
...
function goBack(){ closeSettings(); if(state.currentScreen === 'homeScreen') return; if(['quizScreen','matchScreen'].includes(state.currentScreen)){ stopQuiz(); stopMatchTimer(); showScreen('menuScreen'); return; } if(state.currentScreen === 'menuScreen'){ showScreen('homeScreen'); return; } showScreen('menuScreen'); }
...
function updateMenuTitle(){
  ...
  el.wordlistTitle.textContent = `單字表 · Bảng từ ${state.currentUnit}`;
}
...
function renderWordlist(){
  ...
  el.wordlistContainer.innerHTML = words.map(word => `
    <button class="word-item" data-speak="${escapeAttr(word.zh)}">
      <div class="word-row-top"><span class="word-zh">${word.zh}</span><span class="word-hearts">${heartText(word)}</span></div>
...
function buildFlashPool(){
  ...
  const others = shuffle(words.filter(w => mistakeCount(w) === 0));
  state.flash.pool = [...priority, ...others];
  if(state.flash.pool.length < 16) state.flash.pool = [...state.flash.pool, ...shuffle(words)].slice(0,16);
}

function renderFlashGrid(){
  const group = sample(state.flash.pool, 16);
  el.flashGrid.innerHTML = group.map((word, idx) => `
    <button class="flash-card" data-word-id="${word.id}" data-index="${idx}">
...
function renderFlashSingle(){
  ...
  const word = queue[state.flash.singleIndex];
...
function advanceSingle(known){
  const word = state.flash.singleQueue[state.flash.singleIndex];
  if(!word) return;
...
function switchFlashView(view){
  ...
  if(view==='grid') renderFlashGrid();
  if(view==='single') setupFlashSingle();
}

async function runCountdown(elm){
  elm.classList.remove('hidden');
  for(const t of ['3','2','1']){ elm.textContent=t; await wait(650); }
  elm.textContent='GO'; await wait(350); elm.classList.add('hidden');
}
...
function startQuiz(wrongOnly=false){
  ...
  if(!state.quiz.queue.length){ finishQuiz(true); return; }
...
function tickQuiz(){
  const pct = Math.max(0, state.quiz.remaining / state.quiz.duration); el.quizTimerFill.style.width = `${pct*100}%`; el.quizTimerText.textContent = `${Math.floor(state.quiz.remaining/60)}:${String(state.quiz.remaining%60).padStart(2,'0')}`;
  if(state.quiz.remaining <= 0){ finishQuiz(false); return; }
  state.quiz.remaining -= 1;
}
function renderQuizQuestion(){
  const total = state.quiz.queue.length;
  if(state.quiz.asked >= total){ finishQuiz(true); return; }
  const word = state.quiz.queue[state.quiz.asked]; state.quiz.current = word;
  el.quizProgressText.textContent = `${state.quiz.asked + 1} / ${total}`;
...
function answerQuiz(answerId){
  ...
  document.querySelectorAll('.quiz-option').forEach(btn => {
    const isCorrect = btn.dataset.answerId === state.quiz.current.id;
    if(isCorrect) btn.classList.add('correct');
    if(btn.dataset.answerId === answerId && !correct) btn.classList.add('wrong');
    btn.disabled = true;
  });
...
function finishQuiz(finishedAll){
  stopQuiz();
  el.quizResultSheet.classList.remove('hidden');
  const total = state.quiz.queue.length || 1;
  const success = finishedAll && state.quiz.asked >= total;
  el.quizResultIcon.textContent = success ? '✓' : '✕';
  el.quizResultTitle.textContent = success ? 'Thành công' : 'Thất bại';
  el.quizResultText.textContent = `Đúng ${state.quiz.score} / ${total}`;
}

function pickMatchWords(){
  const words = getCurrentWords();
  const lastTwo = new Set(state.match.roundHistory.flat());
  let priority = words.filter(w => mistakeCount(w) > 0).sort((a,b)=>mistakeCount(b)-mistakeCount(a));
  let fresh = shuffle(words.filter(w => !lastTwo.has(w.id) && !priority.some(p=>p.id===w.id)));
  let other = shuffle(words.filter(w => !priority.some(p=>p.id===w.id) && !fresh.some(f=>f.id===w.id)));
  return [...priority, ...fresh, ...other].slice(0, 9);
}
function buildMatchBoard(){
  const chosen = pickMatchWords();
  ...
  state.match.selected = null; state.match.pairsDone = 0;
  renderMatchBoard();
}
function renderMatchBoard(){
  el.matchBoard.innerHTML = state.match.board.map((card, idx) => `<button class="match-card ${card.lang} ${card.matched?'matched':''}" data-idx="${idx}">${card.matched?'':card.text}</button>`).join('');
  el.matchProgressText.textContent = `${state.match.pairsDone} / 9`;
}
async function startMatch(){
  stopMatchTimer();
  state.match.remaining = 40; el.matchResultSheet.classList.add('hidden'); buildMatchBoard(); showScreen('matchScreen');
  await runCountdown(el.matchCountdown); state.match.active = true; tickMatch(); state.match.timer = setInterval(tickMatch, 1000);
}
...
function tickMatch(){
  const pct = Math.max(0, state.match.remaining / state.match.duration); el.matchTimerFill.style.width = `${pct*100}%`; el.matchTimerText.textContent = `0:${String(state.match.remaining).padStart(2,'0')}`;
  el.matchTimerFill.classList.remove('warn','danger','blink'); if(state.match.remaining <= 15) el.matchTimerFill.classList.add('warn'); if(state.match.remaining <= 5) el.matchTimerFill.classList.add('danger','blink');
  if(state.match.remaining <= 0){ finishMatch(false); return; } state.match.remaining -= 1;
}
function tapMatch(idx){
  if(!state.match.active) return;
  const card = state.match.board[idx]; if(!card || card.matched) return;
  if(state.match.selected === null){ state.match.selected = idx; renderMatchSelection(); return; }
  if(state.match.selected === idx){ state.match.selected = null; renderMatchSelection(); return; }
  const first = state.match.board[state.match.selected];
  if(first.pairId === card.pairId && first.lang !== card.lang){
    first.matched = true; card.matched = true; state.match.pairsDone += 1; markSuccess(first.word); renderWordlist(); state.match.selected = null; renderMatchBoard(); if(state.match.pairsDone === 9) finishMatch(true);
  } else {
    markMistake(first.word); markMistake(card.word); renderWordlist();
    const btns = document.querySelectorAll('.match-card'); [state.match.selected, idx].forEach(i => btns[i]?.classList.add('wrong'));
    state.match.selected = null; setTimeout(renderMatchBoard, 420);
  }
}
function renderMatchSelection(){
  document.querySelectorAll('.match-card').forEach((btn, idx) => btn.classList.toggle('selected', idx === state.match.selected));
}
function finishMatch(success){
  stopMatchTimer();
  el.matchResultSheet.classList.remove('hidden');
  el.matchResultIcon.textContent = success ? '✓' : '✕';
  el.matchResultTitle.textContent = success ? 'Thành công' : 'Thất bại';
  el.matchResultText.textContent = success ? 'Ghép đúng đủ 9 cặp' : `Đúng ${state.match.pairsDone} / 9 cặp`;
}

function restartCurrent(){
  if(state.currentScreen === 'flashScreen'){ if(state.flash.view === 'grid'){ buildFlashPool(); renderFlashGrid(); } else { setupFlashSingle(); } }
  if(state.currentScreen === 'quizScreen') startQuiz(state.quiz.wrongOnly);
  if(state.currentScreen === 'matchScreen') startMatch();
  if(state.currentScreen === 'wordlistScreen') renderWordlist();
}
...
function bindEvents(){
  ...
  document.querySelectorAll('[data-mode]').forEach(btn => btn.addEventListener('click', () => {
    ...
    if(mode === 'flash'){ buildFlashPool(); switchFlashView('grid'); showScreen('flashScreen'); }
  ...
  document.querySelectorAll('[data-action]').forEach(btn => btn.addEventListener('click', () => {
    ...
    if(action === 'backToMenu'){ stopQuiz(); stopMatchTimer(); showScreen('menuScreen'); }
  }));
  el.settingsBackdrop.addEventListener('click', closeSettings); el.closeSettingsBtn.addEventListener('click', closeSettings);
  el.pinyinToggle.addEventListener('change', e => { state.settings.showPinyinFront = e.target.checked; if(state.currentScreen==='flashScreen'){ if(state.flash.view==='grid') renderFlashGrid(); else renderFlashSingle(); } if(state.currentScreen==='quizScreen') renderQuizQuestion(); });
  el.directionToggle.addEventListener('change', e => { state.settings.reverseDirection = e.target.checked; el.directionHelp.textContent = `Hiện tại: ${dirLabelVi()}`; if(state.currentScreen==='flashScreen'){ if(state.flash.view==='grid') renderFlashGrid(); else renderFlashSingle(); } if(state.currentScreen==='quizScreen' && state.quiz.current) renderQuizQuestion(); });
  el.goHomeBtn.addEventListener('click', ()=> showScreen('homeScreen'));
  ...
  el.flashSingleCard.addEventListener('click', ()=> { if(state.flash.singleDone) return; state.flash.singleFlipped = !state.flash.singleFlipped; renderFlashSingle(); });
  let startX = null; el.flashSingleCard.addEventListener('touchstart', e => startX = e.changedTouches[0].clientX, {passive:true}); el.flashSingleCard.addEventListener('touchend', e => { if(startX === null || state.flash.singleDone) return; const dx = e.changedTouches[0].clientX - startX; if(Math.abs(dx) > 60) advanceSingle(dx > 0); startX = null; }, {passive:true});
  ...
  el.matchRestartBtn.addEventListener('click', startMatch); el.matchMenuBtn.addEventListener('click', ()=> { stopMatchTimer(); showScreen('menuScreen'); });
}
